...
import scipy.misc
import cv2 as cv
from utils.countmethods import make_plots
import re
import csv
...
for nfol in range(1,101):
    ...
    make_plots(pmae1a, pmse1a, pmae1b, pmse1b, pmae1c, pmse1c, pmae1d, pmse1d,
               ...
               maea4, mrea4, maeb4, mreb4, maec4, mrec4, maed4, mred4)
'''
with open("datos_MAE_vs_MSE.csv", "w") as csv_file:   
    writer = csv.writer(csv_file, delimiter=',')
    level_counter = 0
    max_levels = len(pmae1a)
    while level_counter < max_levels:
        if(level_counter==0):
            writer.writerow(("MAE Nivel 1 método final","MRE Nivel 1 Método final","MAE Nivel 1 método experimental","MRE Nivel 1 Método experimental", "MAE Nivel 1 método integral","MRE Nivel 1 Método integral", "MAE Nivel 1 método picos","MRE Nivel 1 Método picos","MAE Nivel 2 método final","MRE Nivel 2 Método final","MAE Nivel 2 método experimental","MRE Nivel 2 Método experimental", "MAE Nivel 2 método integral","MRE Nivel 2 Método integral", "MAE Nivel 2 método picos","MRE Nivel 2 Método picos","MAE Nivel 3 método final","MRE Nivel 3 Método final","MAE Nivel 3 método experimental","MRE Nivel 3 Método experimental", "MAE Nivel 3 método integral","MRE Nivel 3 Método integral", "MAE Nivel 3 método picos","MRE Nivel 3 Método picos","MAE Nivel 4 método final","MRE Nivel 4 Método final","MAE Nivel 4 método experimental","MRE Nivel 4 Método experimental", "MAE Nivel 4 método integral","MRE Nivel 4 Método integral", "MAE Nivel 4 método picos","MRE Nivel 4 Método picos","MAE total método final","MRE total Método final","MAE total método experimental","MRE total Método experimental", "MAE total método integral","MRE total Método integral", "MAE total método picos","MRE total Método picos")) 
        else:
            writer.writerow((round(pmae1a[level_counter],2),round(pmse1a[level_counter],2),round(pmae1b[level_counter],2),round(pmse1b[level_counter],2),round(pmae1c[level_counter],2),round(pmse1c[level_counter],2),round(pmae1d[level_counter],2),round(pmse1d[level_counter],2),round(pmae2a[level_counter],2),round(pmse2a[level_counter],2),round(pmae2b[level_counter],2),round(pmse2b[level_counter],2),round(pmae2c[level_counter],2),round(pmse2c[level_counter],2),round(pmae2d[level_counter],2),round(pmse2d[level_counter],2),round(pmae3a[level_counter],2),round(pmse3a[level_counter],2),round(pmae3b[level_counter],2),round(pmse3b[level_counter],2),round(pmae3c[level_counter],2),round(pmse3c[level_counter],2),round(pmae3d[level_counter],2),round(pmse3d[level_counter],2),round(pmae4a[level_counter],2),round(pmse4a[level_counter],2),round(pmae4b[level_counter],2),round(pmse4b[level_counter],2),round(pmae4c[level_counter],2),round(pmse4c[level_counter],2),round(pmae4d[level_counter],2),round(pmse4d[level_counter],2),round(pmae_totala[level_counter],2),round(pmse_totala[level_counter],2),round(pmae_totalb[level_counter],2),round(pmse_totalb[level_counter],2),round(pmae_totalc[level_counter],2),round(pmse_totalc[level_counter],2),round(pmae_totald[level_counter],2),round(pmse_totald[level_counter],2)))
        level_counter = level_counter +1 

'''

with open("histograma_ocupacion.csv", "w") as csv_file:   
    writer = csv.writer(csv_file, delimiter=',')
    writer.writerow(("Nivel","Frames"))
    writer.writerow((1,a))  
    writer.writerow((2,b))
    writer.writerow((3,c))
    writer.writerow((4,d))
...
with open("histograma_MAE.csv", "w") as csv_file:   
    writer = csv.writer(csv_file, delimiter=',')
    writer.writerow(("MAE"))
    level_counter = 0
    max_levels = len(maec)
    while level_counter < max_levels:
        writer.writerow((maec[level_counter],1))
        level_counter = level_counter +1 

with open("histograma_MRE.csv", "w") as csv_file:   
    writer = csv.writer(csv_file, delimiter=',')
    writer.writerow(("MRE"))
    level_counter = 0
    max_levels = len(mrec)
    while level_counter < max_levels:
        writer.writerow((round(mrec[level_counter],0),1))
        level_counter = level_counter +1 
...
print('\n PMAE1_final: %0.2f, PMRE1_final: %0.2f' % (pmae1a,pmse1a))
print('\n PMAE1_experimental: %0.2f, PMRE1_experimental: %0.2f' % (pmae1b,pmse1b))
print('\n PMAE1_integral: %0.2f, PMRE1_integral: %0.2f' % (pmae1c,pmse1c))
print('\n PMAE1_picos: %0.2f, PMRE1_picos: %0.2f' % (pmae1d,pmse1d))

print('\n PMAE2_final: %0.2f, PMRE2_final: %0.2f' % (pmae2a,pmse2a))
print('\n PMAE2_experimental: %0.2f, PMRE2_experimental: %0.2f' % (pmae2b,pmse2b))
print('\n PMAE2_integral: %0.2f, PMRE2_integral: %0.2f' % (pmae2c,pmse2c))
print('\n PMAE2_picos: %0.2f, PMRE2_picos: %0.2f' % (pmae2d,pmse2d))

print('\n PMAE3_final: %0.2f, PMRE3_final: %0.2f' % (pmae3a,pmse3a))
print('\n PMAE3_experimental: %0.2f, PMRE3_experimental: %0.2f' % (pmae3b,pmse3b))
print('\n PMAE3_integral: %0.2f, PMRE3_integral: %0.2f' % (pmae3c,pmse3c))
print('\n PMAE3_picos: %0.2f, PMRE3_picos: %0.2f' % (pmae3d,pmse3d))

print('\n PMAE4_final: %0.2f, PMRE4_final: %0.2f' % (pmae4a,pmse4a))
print('\n PMAE4_experimental: %0.2f, PMRE4_experimental: %0.2f' % (pmae4b,pmse4b))
print('\n PMAE4_integral: %0.2f, PMRE4_integral: %0.2f' % (pmae4c,pmse4c))
print('\n PMAE4_picos: %0.2f, PMRE4_picos: %0.2f' % (pmae4d,pmse4d))

print('\n PMAE_total_final: %0.2f, PMRE_total_final: %0.2f' % (pmae_totala,pmse_totala))
print('\n PMAE_total_experimental: %0.2f, PMRE_total_experimental: %0.2f' % (pmae_totalb,pmse_totalb))
print('\n PMAE_total_integral: %0.2f, PMRE_total_integral: %0.2f' % (pmae_totalc,pmse_totalc))
print('\n PMAE_total_picos: %0.2f, PMRE_total_picos: %0.2f' % (pmae_totald,pmse_totald))

f.write('PMAE: %0.2f, PMRE: %0.2f' % (pmae_totala,pmse_totala))
f.close()
